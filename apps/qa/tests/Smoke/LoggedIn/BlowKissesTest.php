...
#[CoversNothing]
#[Large]
final class BlowKissesTest extends TestCase
{
    #[TestDox('it blocks blow kisses page (`/action.html`) for visitors')]
    public function test_it_blocks_brain_page_for_visitors(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();

        $response = $httpClient->request('GET', '/action.html');

        Assert::blocksPageForLoggedOutVisitors($response);
    }

    #[TestDox('it loads blow kisses page (`/action.html`) for logged in players')]
    public function test_it_loads_brain_page_for_logged_in_players(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();
