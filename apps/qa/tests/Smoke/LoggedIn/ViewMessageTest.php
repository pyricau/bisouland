...
#[CoversNothing]
#[Large]
final class ViewMessageTest extends TestCase
{
    #[TestDox('it blocks view message page (`/lire.html`) for visitors')]
    public function test_it_blocks_view_message_page_for_visitors(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();

        $response = $httpClient->request('GET', '/lire.html');

        Assert::blocksPageForLoggedOutVisitors($response);
    }

    #[TestDox('it loads view message page (`/lire.html`) for logged in players')]
    public function test_it_loads_view_message_page_for_logged_in_players(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();
