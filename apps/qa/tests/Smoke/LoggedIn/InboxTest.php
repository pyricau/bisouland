...
{
    #[TestDox('it blocks inbox page (`/boite.html`) for visitors')]
    public function test_it_blocks_inbox_page_for_visitors(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();

        $response = $httpClient->request('GET', '/boite.html');

        Assert::blocksPageForLoggedOutVisitors($response);
    }

    #[TestDox('it loads inbox page (`/boite.html`) for logged in players')]
    public function test_it_loads_inbox_page_for_logged_in_players(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();
