...
{
    #[TestDox('it blocks account page (`/connected.html`) for visitors')]
    public function test_it_blocks_account_page_for_visitors(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();

        $response = $httpClient->request('GET', '/connected.html');

        Assert::blocksPageForLoggedOutVisitors($response);
    }

    #[TestDox('it loads account page (`/connected.html`) for logged in players')]
    public function test_it_loads_account_page_for_logged_in_players(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();
