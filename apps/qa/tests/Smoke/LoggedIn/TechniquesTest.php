...
#[CoversNothing]
#[Large]
final class TechniquesTest extends TestCase
{
    #[TestDox('it blocks techniques page (`/techno.html`) for visitors')]
    public function test_it_blocks_techniques_page_for_visitors(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();

        $response = $httpClient->request('GET', '/techno.html');

        Assert::blocksPageForLoggedOutVisitors($response);
    }

    #[TestDox('it loads techniques page (`/techno.html`) for logged in players')]
    public function test_it_loads_techniques_page_for_logged_in_players(): void
    {
        $httpClient = TestKernelSingleton::get()->httpClient();
